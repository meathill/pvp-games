...
const DuelSnake = dynamic(() => import('@pvp-games/games').then((mod) => mod.DuelSnakeOnline), {
  ssr: false,
...
function LoadingState() {
  return (
    <div className="flex min-h-100 items-center justify-center rounded-2xl bg-white/80 p-8 shadow-sm ring-1 ring-slate-200 dark:bg-slate-800/80 dark:ring-slate-700">
...
export default function DuelSnakeOnline({ defaultServerUrl }: Props) {
    ...
    <>
      ...
      <div className="rounded-xl bg-slate-50 p-6 dark:bg-slate-800">
        ...
        <ol className="list-inside list-decimal space-y-2 text-sm text-slate-600 dark:text-slate-300">
          ...
          <li>
            在另一个终端运行前端：
            <code className="ml-2 rounded bg-slate-200 px-2 py-1 font-mono text-xs dark:bg-slate-700">pnpm dev</code>
          </li>
